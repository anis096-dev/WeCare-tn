<div wire:init="loadItems">
                ...
                <div class="flex flex-wrap items-center">
                    <div class="relative flex-row flex-1 w-full max-w-full px-4">
                        <div class="flex items-center justify-between">
                            <div>
                                <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-300"> {{ __('treatment.treatments') }}</h3>
                            </div>
                            <div class="ml-4">
                        ...
                        <div class="relative grid grid-cols-6 gap-6 mt-2 ">
                            ...
                            <div class="col-span-3 md:col-span-2 lg:col-span-1">
                                <x-label class="text-xs" for="select"
                                             value="{{ __('app.By') }} {{ __('specialty.specialty') }}"/>
                                <x-select wire:model="specialty" wire:key="specialtyTerm" class="mt-1">
                                    <option value="">{{ __('app.All') }} {{ __('specialty.specialties') }}</option>
                            ...
                            <div class="col-span-3 md:col-span-2 lg:col-span-1">
                                ...
                                <x-select wire:model="orderBy" class="mt-1">
                                    ...
                                    <option value="name">{{ __('treatment.name') }}</option>
                                    <option value="specialty_id">{{ __('treatment.specialty') }}</option>
                                    @if($trashed)
                                        ...
                                        <div class="flex items-center justify-between gap-1 text-sm text-center">
                                            ...
                                            @if($trashed)
                                                @can('restore', $treatment)
                                                    <x-button2 wire:click="selectedItem('restore',{{ $treatment->id }})"
                                                                  class="px-2">
                                                        <x-svg.svg-restore class="w-5 h-5"/>
                                                    </x-button2>
                                                @endcan

                                                @can('forceDelete', $treatment)
                                                    <x-button2
                                                            wire:click="selectedItem('forceDelete',{{ $treatment->id }})"
                                                            class="px-2">
                                            ...
                                            @else
                                                @can('update', $treatment)
                                                    <x-button2 wire:click="selectedItem('update',{{ $treatment->id }})"
                                                                  class="px-2">
                                                ...
                                                @can('view', $treatment)
                                                    <x-button2 wire:click="selectedItem('show',{{ $treatment->id }})"
                                                                  class="px-2">
                                                ...
                                                @can('delete', $treatment)
                                                    <x-button2 wire:click="selectedItem('delete',{{ $treatment->id }})"
                                                                  class="px-2">
    ...
    <livewire:admin.treatment.treatment-create :specialties="$specialties"/>
    <livewire:admin.treatment.treatment-update :specialties="$specialties"/>
    <livewire:admin.treatment.treatment-show/>
    <livewire:admin.treatment.treatment-delete/>
